...
class FuzzedParameter(object):
    ...
    def _uniform(self, state):
        return int(state.uniform(low=self._minval, high=self._maxval + 1))
...
class Fuzzer(common.ExampleGenerator):
    ...
    def take(self, n):
        ...
        for _ in range(n):
            ...
            for t in self._tensors:
                tensor, order = t._make_tensor(params, state)
                tensors[t.name] = tensor
                params[f"{t.name}_order"] = order
            yield common.Example(tensors, None, params)
